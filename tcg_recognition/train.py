...
def fit_model(config, X_train, Y_train, X_test, Y_test, savedir, combination):
    ...
    print("\n\nmodel: {}\n\n".format(config["model"]["name"]))  #naveen
...
def evaluate_model (config,model,X_test,Y_test,savedir,combination):
    ...
    # Calculate accuracy
    accuracy = accuracy_score(predictions_bin[1], predictions_bin[2])
    print("\n\nAccuracy:", accuracy)

    # Calculate F1 score
    f1 = f1_score(predictions_bin[1], predictions_bin[2], labels=classes_tcg, average='weighted')
    print("\n\nF1 score:", f1)
    print("\n\n")
